...
const BuyerSignupPage = () => {
    ...
    const onSubmit = async (data) => {
        ...
        try {
            const response = await authApi.signupBuyer(payload);
    ...
    return (
        <AuthLayout
            ...
            subtitle="Sign up to start sourcing products"
        >
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                ...
                <button
                    type="submit"
                    disabled={isLoading}
                    className="w-full py-3 px-4 bg-gradient-to-r from-brand-600 to-brand-700 hover:from-brand-700 hover:to-brand-800 text-white rounded-xl font-medium shadow-lg shadow-brand-500/30 flex items-center justify-center gap-2 transition-all mt-6 disabled:opacity-70 disabled:cursor-not-allowed"
                >
