    ...
    const fetchMessages = async (pageNum, isInitial = false) => {
        try {
            ...
            setMessages(prev => {
                ...
                // If initial, just set. If loading more (older), prepend.
                // Note: API returns newest first usually? 
                // Need to verify backend sort. Assuming backend returns DESC (newest first).
                // So page 0 is newest. page 1 is older.
                // We typically render ASC (oldest to newest).
                // So we assume `data.content` is sorted? 
                // Let's assume standard Spring Data Pageable: content matches sort.
                // If sort=createdAt,desc -> [Newest, ..., Oldest]

                // We want to store everything in one array.
                // If initial: setMessages(newMsgs)
                // If loading more (older): setMessages([...prev, ...newMsgs]) ?? No.
                // If we load page 1 (older), we append to the "older" end? 
                // Actually MessageList sorts them again by date ASC.
                // So we just need to concat unique messages.
            ...
            sidebar={
                <>
                    ...
                    <ChatList
                        ...
                        activeChatId={chatId}
                        onSelect={(id) => navigate(`/buyer/inquiries/${id}`)}
                    />
                </>
            }
            content={
                <>
                    <ChatHeader
                        ...
                        onBack={() => navigate('/buyer/inquiries')}
                    />
                    <MessageList
                        messages={messages}
...
export default BuyerInquiryChatPage;
